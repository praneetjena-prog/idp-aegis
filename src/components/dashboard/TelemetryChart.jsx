    ...
    <div className="w-full overflow-hidden">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] text-[#3E4650] uppercase tracking-wider">Actual Sensor Demand</span>
          </div>
        </div>
        {mode === 'fault' && (
          <span className="font-mono text-[10px] px-2 py-0.5 rounded bg-[#B07B1C]/10 text-[#B07B1C] border border-[#B07B1C]/20">ANOMALY ZONE • 06:40–14:20</span>
        )}
      </div>
      <div className="relative bg-[#F1EDE6] border border-[#E6E0D6] rounded-lg p-1">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-[240px]">
          ...
          {/* actual line */}
          <path d={actualPath} fill="none" stroke={mode === 'fault' ? "#B07B1C" : "#2C6E9B"} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
      ...
      <div className="mt-2 flex gap-3 font-mono text-[10px] text-[#8A8175]">
        <span>Envelope Deviation: <span className={mode==='fault' ? 'text-[#B07B1C]' : 'text-[#2E7D5B]'}>{mode==='fault' ? '+6.4% • FLAGGED' : '+0.8% • NOMINAL'}</span></span>
        <span>•</span>
